...
4. Claude will then verify the generated code by running `WINDOWS_CARGO_PATH check` to find compile-time errors and `WINDOWS_CARGO_PATH run` to catch runtime failures. If any issues are discovered, Claude will automatically correct the code and repeat this check-run-correct cycle until the code is verified.
