...
`4f1d8592`; DDGI voxel-visibility and terrain-query one-time paths, CPU-updated tracer/DDGI
uniform buffers, and CPU-filled tracer graphics instance buffers now declare Buffer uses; explicit
vertex/index input semantics are available, while static mesh input declarations, frame-wide Image
tracking, and render-pass graphics declarations remain incomplete)
