...
- **历史实验（现已暂停并从预览移除）**：叶片材质另绘不受光照影响的叶面／叶脉／叶柄／叶背基色 pass，蝴蝶使用用户选的翼色；算法围绕各基色生成 0.72～1.28 倍线性亮度的阴影／高光并选取最近档，不会自动把非黑基色变成纯黑。档数 N 指每个局部基色的 N 个明暗档，N=1 只用基色。细线边缘混色可能产生额外 RGB 值，因此并非整张图只用 N 种颜色。这是适应本项目可编辑材料与配色预设的设计选择，不是上述工具文档规定的唯一行业标准。
...
- 实验期间量化在原始采样与新增覆盖像素合并之后统一作用于右侧显示结果；目前量化阶段已经移除。保留原中心采样的几何覆盖规则与是否改变其**显示颜色**是两个独立决策。
...
- **后续结论**：曾将主界面缩为一个“动态色阶（最终颜色）”滑杆，让叶片光照连续计算后统一映射色带；但对蝴蝶深阴影和浅蓝层次的保留不理想。用户决定先不做量化，已从网页预览移除这套控制和额外着色 pass。以后重新研究时，须避免两个档数简单绑定导致双重量化，并评估从实际渲染色分布生成稳定调色板的方法。
