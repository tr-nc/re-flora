...
```text
precompiled 59 shaderc GLSL, 0 Slang GLSL, and 17 native Slang shaders into SPIR-V artifacts
```
...
The independent feature, two-entry `slang-surface` aggregate, and full `slang-validation` aggregate pass the automatic GLSL-reference ABI gate. All 152 aggregate reflection and optimized artifacts pass `spirv-val --target-env vulkan1.3`; an aggregate 2400x1350 hidden release smoke run completed on an NVIDIA RTX 3060 Ti without validation errors or panics.
