...
- `buffer_setup.comp` and `buffer_update.comp` are native under independent contree features. They share contree-build layouts and dimension helpers with leaf writing, preserve their seven- and two-descriptor ABIs and 1x1x1 workgroups, and produce matching node/leaf workloads in hidden tree benchmarks. The 28-sample local medians were 9 us for setup and 2 us for update with both frontends; these state passes are correctness-sensitive but too short for meaningful isolated performance conclusions.
