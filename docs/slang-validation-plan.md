...
- `buffer_setup.comp` is native under `slang-contree-buffer-setup`. It shares contree-build layouts with leaf writing, preserves the seven descriptors and 1x1x1 workgroup, and produces matching node/leaf workloads in the hidden tree benchmark. The 28-sample local median was 9 us for both frontends; this initialization pass is correctness-sensitive but too short for meaningful isolated performance conclusions.
