...
Three seams were compared for proving that the opaque state remains owner-issued-only: it is not
`Copy`, `Clone`, or `Default`.
...
directly after the canonical owner struct, with `::core::marker::Copy`, `::core::clone::Clone`, and
`::core::default::Default` named absolutely: rustc owns alias resolution, local name shadowing,
derives, manual implementations, generic target identity, and every normal build configuration.
This prevents absence/default construction from bypassing the owner-issued initial capability. The
source checker only guards the placement and exact absolute macro/target/trait paths of that
...
module-root rustc-backed non-`Copy`/non-`Clone`/non-`Default` assertion immediately after the
canonical state without attempting to reproduce Rust trait semantics.
