...
候选的 CPU 检查已覆盖固定姿态下降、倾斜镜像滑移、旋转升力、20–240 FPS 恒定风轨迹、120 秒稳定性、极端阵风/停顿、开关保留、槽位复用、寿命下沉/销毁及其他粒子隔离；Slang 生产姿态函数通过 81 个姿态的等边/正交/法线一致性检查。实机与连续帧验收另记，CPU 结果不代表视觉或性能通过。
