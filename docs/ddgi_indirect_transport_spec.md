...
`31-36 ms` (median `34.5 ms`, p95 `36 ms`). These measurements predate localized recovery and are
historical response-latency evidence only; the retained two-stage baseline observations were
`87/88 ms` (median `87.5 ms`), and publication bookkeeping was `0.0095 ms` median. Under the historical 64-epoch policy,
a five-second static portal run recorded zero scheduler claims after `Converged e63`; that demonstrates
sleeping behavior, not the current
