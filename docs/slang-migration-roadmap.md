...
| --- | ---: | --- |
| Native Slang complete | 40 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 36 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
36 shaderc GLSL + 0 Slang GLSL + 40 native Slang = 76 entry points
```
...
- [ ] Finish tracer compute family: 4 GLSL-only entry points remain.
...
| Production source layout | All 91 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
13. **Finish tracer compute**: every non-cloud tracer compute entry is native. Complete the remaining four-entry cloud subfamily.
