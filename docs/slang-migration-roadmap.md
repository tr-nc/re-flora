...
| --- | ---: | --- |
| Native Slang complete | 37 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 39 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
39 shaderc GLSL + 0 Slang GLSL + 37 native Slang = 76 entry points
```
...
- [ ] Finish tracer compute family: 7 GLSL-only entry points remain.
...
| Production source layout | All 86 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
13. **Finish tracer compute**: the VSM, leaf-shadow, and lens-flare compute subfamilies are native. Migrate the remaining 7 entries across cloud, god-ray, terrain-query, and wind-volume groups.
