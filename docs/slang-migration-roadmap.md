...
| --- | ---: | --- |
| Native Slang complete | 59 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 17 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
17 shaderc GLSL + 0 Slang GLSL + 59 native Slang = 76 entry points
```
...
- [ ] Migrate chunk writer: 15/21 native; 6 entry points remain.
...
### Builder: chunk writer — 15/21 native
...
| Production source layout | All 114 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
14. **Migrate chunk writer**: region setup, sampling, smoothing, brushing, and moisture spread are native; port the remaining 6 entries in small behavior-related groups.
