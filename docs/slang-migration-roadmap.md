...
| --- | ---: | --- |
| Native Slang complete | 44 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 32 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
32 shaderc GLSL + 0 Slang GLSL + 44 native Slang = 76 entry points
```
...
| Production source layout | All 95 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
14. **Migrate chunk writer**: port its 21 entries in small behavior-related groups.
