...
| --- | ---: | --- |
| Native Slang complete | 45 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 31 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
31 shaderc GLSL + 0 Slang GLSL + 45 native Slang = 76 entry points
```
...
- [ ] Migrate chunk writer: 1/21 native; 20 entry points remain.
...
### Builder: chunk writer — 1/21 native
...
| Production source layout | All 97 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
14. **Migrate chunk writer**: region indirect setup is native; port the remaining 20 entries in small behavior-related groups.
