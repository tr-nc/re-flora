...
| --- | ---: | --- |
| Native Slang complete | 48 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 28 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
28 shaderc GLSL + 0 Slang GLSL + 48 native Slang = 76 entry points
```
...
- [ ] Migrate chunk writer: 4/21 native; 17 entry points remain.
...
### Builder: chunk writer — 4/21 native
...
| Production source layout | All 101 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
14. **Migrate chunk writer**: region setup, MBO initialization, and both diffusion directions are native; port the remaining 17 entries in small behavior-related groups.
