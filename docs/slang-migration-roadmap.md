...
| --- | ---: | --- |
| Native Slang complete | 50 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 26 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
26 shaderc GLSL + 0 Slang GLSL + 50 native Slang = 76 entry points
```
...
- [ ] Migrate chunk writer: 6/21 native; 15 entry points remain.
...
### Builder: chunk writer — 6/21 native
...
| Production source layout | All 103 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
14. **Migrate chunk writer**: region setup and the complete five-entry MBO smoother are native; port the remaining 15 entries in small behavior-related groups.
