...
| --- | ---: | --- |
| Native Slang complete | 62 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 14 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
14 shaderc GLSL + 0 Slang GLSL + 62 native Slang = 76 entry points
```
...
- [ ] Migrate chunk writer: 18/21 native; 3 entry points remain.
...
### Builder: chunk writer — 18/21 native
...
| Production source layout | All 117 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
14. **Migrate chunk writer**: region setup, heightmap/classification, sampling, smoothing, and all soil passes except drying are native; port the remaining 3 entries in small behavior-related groups.
