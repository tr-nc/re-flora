...
| --- | ---: | --- |
| Native Slang complete | 34 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 42 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
42 shaderc GLSL + 0 Slang GLSL + 34 native Slang = 76 entry points
```
...
- [ ] Finish tracer compute family: 10 GLSL-only entry points remain.
...
| Production source layout | All 83 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
13. **Finish tracer compute**: the VSM and leaf-shadow compute subfamilies are native. Migrate the remaining 10 entries across cloud, lens-flare, god-ray, terrain-query, and wind-volume groups.
