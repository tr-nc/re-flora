...
| --- | ---: | --- |
| Native Slang complete | 55 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 21 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
21 shaderc GLSL + 0 Slang GLSL + 55 native Slang = 76 entry points
```
...
- [ ] Migrate chunk writer: 11/21 native; 10 entry points remain.
...
### Builder: chunk writer — 11/21 native
...
| Production source layout | All 109 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
14. **Migrate chunk writer**: region setup, all three sampling utilities, height extraction/target smoothing, and the complete five-entry MBO smoother are native; port the remaining 10 entries in small behavior-related groups.
