...
| --- | ---: | --- |
| Native Slang complete | 61 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 15 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
15 shaderc GLSL + 0 Slang GLSL + 61 native Slang = 76 entry points
```
...
- [ ] Migrate chunk writer: 17/21 native; 4 entry points remain.
...
### Builder: chunk writer — 17/21 native
...
| Production source layout | All 116 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
14. **Migrate chunk writer**: region setup, atlas classification, sampling, smoothing, and all soil passes except drying are native; port the remaining 4 entries in small behavior-related groups.
