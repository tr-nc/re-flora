...
| --- | ---: | --- |
| Native Slang complete | 42 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 34 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
34 shaderc GLSL + 0 Slang GLSL + 42 native Slang = 76 entry points
```
...
- [ ] Finish tracer compute family: 2 GLSL-only entry points remain.
...
| Production source layout | All 93 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
13. **Finish tracer compute**: both cloud temporal resolves are native. Complete raw visible-cloud and cloud-shadow generation.
