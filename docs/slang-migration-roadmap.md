...
| --- | ---: | --- |
| Native Slang complete | 31 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 45 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
45 shaderc GLSL + 0 Slang GLSL + 31 native Slang = 76 entry points
```
...
- [ ] Finish tracer compute family: 13 GLSL-only entry points remain.
...
| Production source layout | All 80 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
13. **Finish tracer compute**: shadow-depth copy, EVSM creation, and horizontal blur are native. Migrate the remaining 13 entries, beginning with vertical VSM blur, then leaf-shadow, cloud, reflection, lighting, and utility groups.
