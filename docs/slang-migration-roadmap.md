...
| --- | ---: | --- |
| Native Slang complete | 32 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 44 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
44 shaderc GLSL + 0 Slang GLSL + 32 native Slang = 76 entry points
```
...
- [ ] Finish tracer compute family: 12 GLSL-only entry points remain.
...
| Production source layout | All 81 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
13. **Finish tracer compute**: the four-entry VSM subfamily is native. Migrate the remaining 12 entries, beginning with leaf-shadow, then cloud, reflection, lighting, and utility groups.
