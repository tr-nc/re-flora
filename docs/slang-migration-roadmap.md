...
| --- | ---: | --- |
| Native Slang complete | 38 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 38 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
38 shaderc GLSL + 0 Slang GLSL + 38 native Slang = 76 entry points
```
...
- [ ] Finish tracer compute family: 6 GLSL-only entry points remain.
...
| Production source layout | All 88 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
13. **Finish tracer compute**: the VSM, leaf-shadow, lens-flare, and wind-volume compute paths are native. Migrate the remaining 6 entries across cloud, god-ray, and terrain-query groups.
