...
| --- | ---: | --- |
| Native Slang complete | 47 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 29 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
29 shaderc GLSL + 0 Slang GLSL + 47 native Slang = 76 entry points
```
...
- [ ] Migrate chunk writer: 3/21 native; 18 entry points remain.
...
### Builder: chunk writer — 3/21 native
...
| Production source layout | All 100 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
14. **Migrate chunk writer**: region setup, MBO initialization, and A-to-B diffusion are native; port the remaining 18 entries in small behavior-related groups.
