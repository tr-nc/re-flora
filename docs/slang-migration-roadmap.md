...
| --- | ---: | --- |
| Native Slang complete | 58 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 18 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
18 shaderc GLSL + 0 Slang GLSL + 58 native Slang = 76 entry points
```
...
- [ ] Migrate chunk writer: 14/21 native; 7 entry points remain.
...
### Builder: chunk writer — 14/21 native
...
| Production source layout | All 113 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
14. **Migrate chunk writer**: region setup, sampling, smoothing, and moisture/fertility brushing are native; port the remaining 7 entries in small behavior-related groups.
