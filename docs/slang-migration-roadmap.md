...
| --- | ---: | --- |
| Native Slang complete | 29 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 47 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
47 shaderc GLSL + 0 Slang GLSL + 29 native Slang = 76 entry points
```
...
- [ ] Finish tracer compute family: 15 GLSL-only entry points remain.
...
| Production source layout | All 76 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
13. **Finish tracer compute**: shadow-depth copy is native. Migrate the remaining 15 entries in focused VSM, leaf-shadow, cloud, reflection, lighting, and utility groups while reusing accepted tracer modules.
