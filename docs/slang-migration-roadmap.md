...
| --- | ---: | --- |
| Native Slang complete | 43 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 33 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
33 shaderc GLSL + 0 Slang GLSL + 43 native Slang = 76 entry points
```
...
- [ ] Finish tracer compute family: 1 GLSL-only entry point remains.
...
| Production source layout | All 94 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
13. **Finish tracer compute**: visible-cloud generation and both temporal resolves are native. Complete raw cloud-shadow generation and add the cloud aggregate feature.
