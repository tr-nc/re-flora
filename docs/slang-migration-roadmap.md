...
| --- | ---: | --- |
| Native Slang complete | 51 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 25 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
25 shaderc GLSL + 0 Slang GLSL + 51 native Slang = 76 entry points
```
...
- [ ] Migrate chunk writer: 7/21 native; 14 entry points remain.
...
### Builder: chunk writer — 7/21 native
...
| Production source layout | All 104 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
14. **Migrate chunk writer**: region setup, removal sampling, and the complete five-entry MBO smoother are native; port the remaining 14 entries in small behavior-related groups.
