...
| --- | ---: | --- |
| Native Slang complete | 75 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 1 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
1 shaderc GLSL + 0 Slang GLSL + 75 native Slang = 76 entry points
```
...
- [x] Migrate chunk writer: all 21 entry points are native under `slang-chunk-writer`.
...
- [x] Migrate particles: all 3 entries are native under `slang-particles`.
- [x] Migrate sprinkler props: the single vertex entry is native and its fragment-stage pairing is verified.
- [x] Migrate terrarium glass: both vertex and fragment entries are native under `slang-terrarium-glass`.
...
### Builder: chunk writer — 21/21 native
...
- [x] `shader/builder/chunk_writer/terrain_moisture_dry.comp` — `slang-chunk-writer-terrain-moisture-dry`; `slang-chunk-writer` enables all 21 chunk-writer entries
...
- [x] `shader/foliage/leaves_lod.vert` — `slang-foliage-leaves-lod`
- [x] `shader/foliage/leaves_shadow.frag` — `slang-foliage-leaves-shadow-frag`
- [x] `shader/foliage/leaves_shadow.vert` — `slang-foliage-leaves-shadow-vert`; `slang-foliage-leaves-shadow` enables the pair
- [x] `shader/foliage/leaves.vert` — `slang-foliage-leaves-vert`; shared main-pass logic lives in `flora_vertex.slang` and surface-only growth resources in `surface_flora_vertex.slang`

### Particles — 3/3 native

- [x] `shader/particles/particle_lod_textured.frag` — `slang-particles-lod-textured-frag`
- [x] `shader/particles/particle_lod_textured.vert` — `slang-particles-lod-textured-vert`; `slang-particles-lod-textured` enables the pair and `slang-particles` the family
- [x] `shader/particles/water_droplet.frag` — `slang-particles-water-droplet`

### Props — 1/1 native

- [x] `shader/props/sprinkler.vert` — `slang-sprinkler`

### Terrarium — 2/2 native

- [x] `shader/terrarium/glass.frag` — `slang-terrarium-glass-frag`; `slang-terrarium-glass` enables both stages
- [x] `shader/terrarium/glass.vert` — `slang-terrarium-glass-vert`
...
| Production source layout | All 132 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
15. **Continue graphics migration**: sprinkler, particles, and terrarium glass are native; finish foliage.
