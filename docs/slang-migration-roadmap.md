...
| --- | ---: | --- |
| Native Slang complete | 54 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 22 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
22 shaderc GLSL + 0 Slang GLSL + 54 native Slang = 76 entry points
```
...
- [ ] Migrate chunk writer: 10/21 native; 11 entry points remain.
...
### Builder: chunk writer — 10/21 native
...
| Production source layout | All 108 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
14. **Migrate chunk writer**: region setup, all three sampling utilities, height extraction, and the complete five-entry MBO smoother are native; port the remaining 11 entries in small behavior-related groups.
