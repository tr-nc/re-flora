...
| --- | ---: | --- |
| Native Slang complete | 30 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 46 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
46 shaderc GLSL + 0 Slang GLSL + 30 native Slang = 76 entry points
```
...
- [ ] Finish tracer compute family: 14 GLSL-only entry points remain.
...
| Production source layout | All 78 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
13. **Finish tracer compute**: shadow-depth copy and EVSM creation are native. Migrate the remaining 14 entries, beginning with the two VSM blur passes, then leaf-shadow, cloud, reflection, lighting, and utility groups.
