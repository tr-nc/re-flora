...
| --- | ---: | --- |
| Native Slang complete | 49 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 27 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
27 shaderc GLSL + 0 Slang GLSL + 49 native Slang = 76 entry points
```
...
- [ ] Migrate chunk writer: 5/21 native; 16 entry points remain.
...
### Builder: chunk writer — 5/21 native
...
| Production source layout | All 102 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
14. **Migrate chunk writer**: region setup and MBO initialization/diffusion/scoring are native; port the remaining 16 entries in small behavior-related groups.
