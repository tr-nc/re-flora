...
| Production source layout | All 49 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
9. **Complete contree construction**: Phase 3 is in progress. Native buffer setup, level update, and final-buffer update preserve their seven-, two-, and five-binding ABIs, initialize the same state, advance indirect dispatches, and seed final concatenation identically. They produce matching tree-benchmark workloads and are independently gated. Port `tree_write.comp` and `concat.comp` as separately validated units.
