...
| --- | ---: | --- |
| Native Slang complete | 33 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 43 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
43 shaderc GLSL + 0 Slang GLSL + 33 native Slang = 76 entry points
```
...
- [ ] Finish tracer compute family: 11 GLSL-only entry points remain.
...
| Production source layout | All 82 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
13. **Finish tracer compute**: the VSM subfamily and leaf-shadow temporal pass are native. Migrate the remaining 11 entries, beginning with the leaf-shadow mask, then cloud, reflection, lighting, and utility groups.
