...
| --- | ---: | --- |
| Native Slang complete | 46 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 30 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
30 shaderc GLSL + 0 Slang GLSL + 46 native Slang = 76 entry points
```
...
- [ ] Migrate chunk writer: 2/21 native; 19 entry points remain.
...
### Builder: chunk writer — 2/21 native
...
| Production source layout | All 99 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
14. **Migrate chunk writer**: region indirect setup and MBO density initialization are native; port the remaining 19 entries in small behavior-related groups.
