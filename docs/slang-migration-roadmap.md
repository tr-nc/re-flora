...
| --- | ---: | --- |
| Native Slang complete | 41 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 35 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
35 shaderc GLSL + 0 Slang GLSL + 41 native Slang = 76 entry points
```
...
- [ ] Finish tracer compute family: 3 GLSL-only entry points remain.
...
| Production source layout | All 92 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
13. **Finish tracer compute**: every non-cloud tracer compute entry and cloud-shadow temporal resolve are native. Complete the remaining three cloud entries.
