...
| --- | ---: | --- |
| Native Slang complete | 68 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 8 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
8 shaderc GLSL + 0 Slang GLSL + 68 native Slang = 76 entry points
```
...
| Production source layout | All 123 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
