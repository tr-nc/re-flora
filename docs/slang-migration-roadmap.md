...
| --- | ---: | --- |
| Native Slang complete | 39 | Native source is independently selectable and has passed local gates |
| Slang backend only | 0 | Existing GLSL compiles through Slang; native rewrite remains TODO |
| GLSL only | 37 | No completed Slang replacement yet |

Current aggregate `slang-validation` build:

```text
37 shaderc GLSL + 0 Slang GLSL + 39 native Slang = 76 entry points
```
...
- [ ] Finish tracer compute family: 5 GLSL-only entry points remain.
...
| Production source layout | All 89 accepted modules and entries live under `shader/slang/`; runtime logical paths remain unchanged | Keep native production sources in this root as families migrate |
...
13. **Finish tracer compute**: all non-cloud utilities except god-ray are native. Migrate the remaining 5 entries: the four cloud passes and god-ray.
