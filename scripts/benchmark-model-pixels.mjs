...
const help=`Usage: node scripts/benchmark-model-pixels.mjs [--seconds 8] [--binary target/release/re-flora] [--output target/model-pixel-bench] [--stress-leaves 0] [--suite apples|stage-one|display]
...
interval within 5% of the 8px reference (stage-one: default 16 views; display: A) or 60 Hz, whichever is slower.
...
if(!['apples','stage-one','display'].includes(options.suite)){console.error(`--suite must be apples, stage-one or display.\n${help}`);process.exit(2);}
...
const results=[];
try {
 const cases=options.suite==='display'?[false,true].map(screenGrid=>({n:32,views:16,screenGrid,label:screenGrid?'B-screen':'A-rotating'})):
 options.suite==='stage-one'?[8,16,128,512].map(views=>({
  n:32,views,screenGrid:false,label:`views-${views}`
 })):[8,32,64].map(n=>({n,views:16,screenGrid:false,label:n}));
 for(const scene of ['attached','fallen'])for(const {n,views,screenGrid,label} of cases){
  ...
  owned=setting(setting(owned,'model_pixel_view_count',views),'model_pixel_screen_grid',screenGrid);
...
assert.ok(results.filter(r=>r.resolution===32).every(r=>{
 const baseline=results.find(b=>b.scene===r.scene&&(options.suite==='display'?!b.screen_grid:options.suite==='stage-one'?b.views===16:b.resolution===8));
