    ...
    "check_ddgi_correctness.sh": frozenset(
        {
            ...
            "scripts/lib/capture_process_evidence.sh",
        }
    ),
    ...
    "check_ddgi_lifecycle_acceptance.sh": frozenset(
        {
            "scripts/analyze_current_environment_irradiance_capture.py",
            "scripts/lib/capture_process_evidence.sh",
            "scripts/validate_ddgi_density_lifecycle.py",
...
PRODUCTION_AUXILIARY_DEPENDENCIES: dict[str, frozenset[str]] = {
    "scripts/lib/capture_process_evidence.sh": frozenset(
...
def production_evidence_dependencies(sources: dict[str, str]) -> frozenset[str]:
    ...
    pending = [f"scripts/{name}" for name in sources]
    visited: set[str] = set()
    dependencies: set[str] = set(pending)
    while pending:
        dependency_path = pending.pop()
        if dependency_path in visited:
            continue
        visited.add(dependency_path)
        runner_name = dependency_path.removeprefix("scripts/")
        declared = RUNNER_PRODUCTION_DEPENDENCIES.get(
            runner_name,
            PRODUCTION_AUXILIARY_DEPENDENCIES.get(
                dependency_path, frozenset()
            ),
        )
            ...
            if basename == "cargo":
                ...
                if not (
                    ...
                    in (
                        ...
                        ("$density_validator", "$console", "--identity-tsv"),
                    )
                ):
