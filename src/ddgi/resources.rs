...
impl DdgiRelocationReadbackStats {
    fn from_array(values: [u32; DDGI_RELOCATION_STATS_COUNT]) -> Self {
        Self {
            ...
            local: values[4],
            local_target: values[5],
            local_best_effort: values[6],
            full_search: values[7],
            clearance_sum: values[8],
            distance_squared_twice_sum: values[9],
            moved: values[10],
            clearance_1: values[11],
            clearance_2: values[12],
            clearance_3_plus: values[13],
        }
    }
}
