...
mod tests {
    ...
    fn authored_input(snapshot: DdgiRadianceSnapshot) -> AuthoredEnvironmentLightingInput {
        AuthoredEnvironmentLightingInput {
            ...
            glass_material_revision: snapshot.glass_material_revision,
    ...
    fn large_sun_and_material_discontinuities_publish_immediately() {
        ...
        snapshot.glass_experiment_enabled = true;
        snapshot.glass_material_revision = 1;
        let glass_step = lighting_at(4, Duration::from_millis(40), snapshot);
        let glass_step = runtime.observe_authored_lighting(glass_step);
        assert!(glass_step.transport_published);
        assert_eq!(glass_step.transport.revision(), 4);
        assert_eq!(glass_step.transport.snapshot().glass_material_revision, 1);
        assert_eq!(
            glass_step.transport.change().reason,
            DdgiRadianceChangeReason::TransportInputStep
        );
        assert!(glass_step.transport.change().resets_irradiance_history());
    }
