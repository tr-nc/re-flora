...
use super::{
    ...
    glass_voxel_test_scene::{GlassVoxelTestScene, TestScenePhase as GlassTestScenePhase},
...
use crate::cli::{
    AutomationPlan, CameraAutomation, GlassCoverage, GlassDebugView, LightingModeAcceptanceOptions,
    Scenario,
};
...
pub(super) enum StandardScenarioOwner {
    ...
    LightingModeAcceptance(Option<LightingModeAcceptanceOptions>),
}
...
impl ScenarioOwner {
    pub(super) fn loading_directive(&self) -> LoadingDirective {
        match self {
            ...
            Self::Standard(StandardScenarioOwner::TestScene(TestSceneOwner::Glass(_))) => {
                LoadingDirective::Glass
            }
            Self::Standard(
                StandardScenarioOwner::Water(WaterScenarioOwner::EditSoak(_))
                | StandardScenarioOwner::TestScene(TestSceneOwner::Hybrid(_))
                | StandardScenarioOwner::LightingModeAcceptance(_),
            )
    ...
    pub(super) fn test_scene_frame_plan(&self) -> TestSceneFramePlan {
        match self {
            ...
            Self::Standard(StandardScenarioOwner::TestScene(TestSceneOwner::Glass(scene))) => {
                TestSceneFramePlan {
                    kind: TestSceneKind::Glass,
                    capture_ready: scene.is_ready(),
                    hides_terrain_edit_preview: false,
                }
            }
...
impl LaunchOwners {
    ...
    pub(super) fn glass_experiment_settings(&self) -> Option<(GlassCoverage, GlassDebugView)> {
        match &self.mode {
            LaunchMode::General {
                scenario:
                    ScenarioOwner::Standard(StandardScenarioOwner::TestScene(TestSceneOwner::Glass(
                        scene,
                    ))),
                ..
            } => Some((scene.coverage(), scene.debug_view())),
            _ => None,
        }
    }
    ...
    pub(super) fn glass_scene_fixed_camera_validation(&self) -> Option<bool> {
        match &self.mode {
            LaunchMode::General {
                scenario:
                    ScenarioOwner::Standard(StandardScenarioOwner::TestScene(TestSceneOwner::Glass(
                ...
                ..
            } => Some(scene.validate_fixed_camera_frame()),
            _ => None,
        }
    }

    pub(super) fn set_glass_scene_phase(&mut self, phase: GlassTestScenePhase) {
        match &mut self.mode {
            LaunchMode::General {
                scenario:
                    ScenarioOwner::Standard(StandardScenarioOwner::TestScene(TestSceneOwner::Glass(
            ...
            _ => panic!("Glass scene phase requires the Glass launch owner"),
        }
    }
    ...
    pub(super) fn take_lighting_mode_acceptance_options(
        &mut self,
    ) -> anyhow::Result<Option<LightingModeAcceptanceOptions>> {
        match &mut self.mode {
            LaunchMode::General {
                scenario:
                    ScenarioOwner::Standard(StandardScenarioOwner::LightingModeAcceptance(options)),
                ..
            } => options.take().map(Some).ok_or_else(|| {
                anyhow::anyhow!("lighting-mode acceptance options were already consumed")
            }),
            _ => Ok(None),
        }
    }
...
pub(in crate::app) fn prepare_startup_owners(
    ...
    if matches!(&scenario, Scenario::LightingModeAcceptance(_))
        && !matches!(&camera, CameraAutomation::None)
    {
        return Err("lighting-mode acceptance cannot carry a second camera automation".to_owned());
    }
    ...
    let (camera, screenshot) = match camera {
        ...
        CameraAutomation::FixedSceneScreenshot { capture } => {
            (CameraOwner::None, ScreenshotRuntime::new(Some(capture)))
        }
    ...
    let mode = match scenario {
        ...
        Scenario::GlassVoxel(options) => LaunchMode::General {
            camera,
            scenario: ScenarioOwner::Standard(StandardScenarioOwner::TestScene(
                TestSceneOwner::Glass(GlassVoxelTestScene::new(
                    options.coverage,
                    options.debug_view,
                    options.validate_fixed_camera_frame,
                )),
            )),
        },
        ...
        Scenario::LightingModeAcceptance(options) => LaunchMode::General {
            camera,
            scenario: ScenarioOwner::Standard(StandardScenarioOwner::LightingModeAcceptance(Some(
                options,
            ))),
        },
...
mod tests {
    ...
    fn owner_kind(launch: LaunchOwners) -> OwnerKind {
        match launch.mode {
            LaunchMode::General { scenario, .. } => match scenario {
                ...
                ScenarioOwner::Standard(StandardScenarioOwner::TestScene(
                    TestSceneOwner::Glass(_),
                )) => OwnerKind::Glass,
                ScenarioOwner::Standard(StandardScenarioOwner::LightingModeAcceptance(_)) => {
                    OwnerKind::LightingModeAcceptance
                }
    ...
    #[test]
    fn lighting_mode_acceptance_options_move_once_into_the_runtime() {
        let mut launch = launch_for(Scenario::LightingModeAcceptance(
            crate::LightingModeAcceptanceOptions {
        ...
        assert_eq!(
            launch
                ...
                .unwrap()
                .artifact_path,
            std::path::PathBuf::from("target/lighting.rflma")
        );
        assert!(launch.take_lighting_mode_acceptance_options().is_err());
    }
    ...
    fn all_fixed_scenarios_construct_their_exhaustive_family_owner() {
        let cases = [
            ...
            (
                Scenario::GlassVoxel(crate::cli::GlassVoxelOptions {
            ...
            (
                Scenario::LightingModeAcceptance(crate::LightingModeAcceptanceOptions {
                    artifact_path: "target/lighting.rflma".into(),
                }),
                OwnerKind::LightingModeAcceptance,
            ),
