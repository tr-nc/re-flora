...
use crate::{
    app::{
        physical_visible_terrain::{PhysicalTerrainBuilders, PhysicalTerrainPublication},
        world_ops,
    },
...
///
/// Atlas mutation and physical Visible Terrain Publication have already completed when this value
/// exists. Runtime uses it only to notify downstream observers of that published world outcome.
#[derive(Clone, Debug)]
...
impl WorldEditTransaction {
    ...
    pub(crate) fn execute(
        ...
        builders: PhysicalTerrainBuilders<'_>,
        voxel_dim_per_chunk: UVec3,
    ) -> Result<Option<WorldEditOutcome>> {
        let Self {
            voxel_edits,
            publication,
        } = self;
        let prepared_publication = match publication {
            WorldEditPublication::DeferredUntilLoading => None,
            WorldEditPublication::Terrain(bound) => Some((
                PhysicalTerrainPublication::from_build_edit(
            ...
            WorldEditPublication::Trees(affected_regions) => {
                ...
                let affected_voxels = chunk_bound(&chunk_ids, voxel_dim_per_chunk)?;
                Some((
                    PhysicalTerrainPublication::from_build_edit(
