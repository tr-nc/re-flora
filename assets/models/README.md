...
The apple A/B uses `apple-source.mjs` as the single geometry recipe for the HTML preview and the generated game render mesh `apple-preview.json` (`node scripts/publish-apple-model.mjs`). The game Debug checkbox defaults to the original voxel apple; when checked, it selects the new red apple for both attached and dropped fruit, including their shadow passes. The collider, fruit growth and drop physics remain voxel-based. The HTML green-apple preset and color pickers do not affect the game.
