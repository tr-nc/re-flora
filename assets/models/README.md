...
The apple A/B uses `apple-source.mjs` as the single geometry recipe for the HTML preview and the generated game render mesh `apple-preview.json` (`node scripts/publish-apple-model.mjs`). The game Debug checkbox defaults to the original voxel apple; when checked, it selects the new red apple for both attached and dropped fruit, including their shadow passes. Both use the shared fixed camera-space N×N model pixel grid and depth/coverage sampling, with their own `Pixels per New Apple` slider (8–64, default 32), independent of butterfly and falling-leaf resolutions. The attached-fruit adapter uses the tree's published wind/attachment pose; dropped fruit uses its rigid-body pose. The collider, fruit growth and drop physics remain voxel-based. The HTML green-apple preset and color pickers do not affect the game.
