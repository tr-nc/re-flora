...
void unpack_vertex_data(out ivec3 o_vox_local_pos, out uvec3 o_vert_offset_in_vox,
    ...
    o_vert_offset_in_vox = uvec3(
        offset_packed & OFFSET_MASK,
        (offset_packed >> BIT_PER_OFFSET) & OFFSET_MASK,
        (offset_packed >> (BIT_PER_OFFSET * 2)) & OFFSET_MASK
    );

    // Extract and normalize gradients
    uint gradient_data = (packed_data >> (POS_BITS + OFFSET_BITS)) & ((1u << BIT_PER_CUSTOM_INFO) - 1u);
