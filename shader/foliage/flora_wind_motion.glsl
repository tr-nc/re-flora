...
const float GRASS_VIBRATION_PRIMARY_HZ = 31.0;
const float GRASS_VIBRATION_SECONDARY_HZ = 47.0;
const float LEAF_PADDLE_AMPLITUDE_VOXELS = 0.80;
const float LEAF_PADDLE_PRIMARY_HZ = 9.0;
const float LEAF_PADDLE_SECONDARY_HZ = 15.0;
...
vec3 grass_wind_vibration(vec3 wind_vec, float wind_gradient, uint instance_seed,
    ...
    float vibration = sin(time * GRASS_VIBRATION_PRIMARY_HZ + phase + height_phase);
    vibration += 0.45 * sin(time * GRASS_VIBRATION_SECONDARY_HZ + phase * 1.37 - height_phase * 0.6);
...
vec3 leaf_wind_paddling(vec3 wind_vec, float wind_gradient, uint instance_seed,
    ...
    float paddle = sin(time * LEAF_PADDLE_PRIMARY_HZ + phase + shell_phase);
    paddle += 0.35 * sin(time * LEAF_PADDLE_SECONDARY_HZ + phase * 1.61 - shell_phase * 0.7);
