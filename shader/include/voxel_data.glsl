...
// Surface words use bits 8..23 for an octahedral RG8 normal. Bits 24..29 are reserved.
const uint VOXEL_NORMAL_BITS_MASK   = 0xFFFFu;
...
vec3 voxel_surface_normal_from_data(uint voxel_data) {
    return unpack_normal_oct16(voxel_normal_bits_from_data(voxel_data));
}
...
uint pack_voxel_surface_data(uint voxel_type, uint normal_bits, uint hash_id) {
    uint voxel_data = 0u;
