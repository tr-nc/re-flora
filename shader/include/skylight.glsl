...
vec3 get_sky_color(vec3 dir) { 
    return srgb_to_linear(vec3(0.5, 0.7, 1.0)); 
}

vec3 get_sky_color_with_sun(vec3 view_dir, vec3 sun_dir, vec3 sun_color, float sun_size) {
    vec3 sky_color = get_sky_color(view_dir);
    ...
    return mix(sky_color, yellow_sun_color, sun_intensity);
}
