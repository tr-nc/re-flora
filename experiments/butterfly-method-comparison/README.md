...
- 历史记录显示导出、循环和重放验证通过；**这不是本次迁入后的 Blender 复跑结论**。
...
1. 检查当前游戏实际取样行、动画速度、源单帧尺寸、世界尺寸与屏幕显示大小，区分源像素密度和显示缩放问题。
2. 复跑保存的 v2 模型，先确认本机导出链路，不重建覆盖手调源文件。
3. 复核已有一手案例，补充纯手绘与混合流程的适用边界；不把三维流畅等同于小尺寸 sprite 好看。
4. 先用同一显示尺寸比较 16/12/8px 候选，优先轮廓与拍翅节奏；这是待试方案，不是已经支持的导出参数。
5. 用户确认视觉方向后再决定游戏接入、批量导出接口及性能验证。若做游戏内 A/B，必须使用 Debug 面板运行时复选框保留原模式。

待讨论：用户觉得“分辨率高”更接近色块太细，还是蝴蝶在画面中偏大？是否接受三维批量导出后少量手工像素修整，还是希望完全自动生成？
