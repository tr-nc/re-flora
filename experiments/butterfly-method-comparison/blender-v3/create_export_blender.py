"""REVIEW-ONLY v3: broad cupped wings and authored silhouettes for native 12px.

Independent recipe; preserves v1/v2. Fixed cameras/canvas, no per-frame auto-fit.
"""
import argparse
import json
import math
...
POSES = {
    ...
    "abdomen_degrees": [5, 0, -7, -3, 7],
    "abdomen_tip_degrees": [8, 2, -10, -4, 10],
}
...
def create():
    ...
    body_volume("Thorax",(0,.075,.18),(.16,.29,.20),flight,mats,1)
    body_volume("Head",(0,.37,.22),(.14,.16,.14),flight,mats,0)
    ...
    scene.frame_set(11)
    scene["PROTOTYPE"]="Butterfly v3: cupped broad wings, restrained pitch, 12px silhouette-first. Fixed world reference."
    ...
    manifest={
        "prototype":True,"version":3,"blender_version":bpy.app.version_string,
        "blend":"butterfly-prototype.blend","glb":"butterfly-prototype.glb",
        "geometry":{"mesh_objects":len(meshes),"faces":sum(len(o.data.polygons) for o in meshes),"wing_design":"Broad cupped two-lobe silhouette, colored rim, no antennae, thickened dark thorax; identical geometry in every view."},
