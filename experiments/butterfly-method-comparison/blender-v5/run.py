#!/usr/bin/env python3
"""Export saved two-surface wing-only v5 source. --rebuild explicitly replaces it from the recipe.
...
def assemble(draft):
    ...
    for i, size in enumerate((12, 16)):
        draw.text((8, i*530+8), f'V5 uniform surfaces {size}px - unlit diagnostic samples', fill='white')
    ...
    material_names = {m['name'] for m in glb['materials']}
    assert material_names == {'Wing upper', 'Wing lower'}, material_names
    for mesh in glb['meshes']:
        assert {glb['materials'][p['material']]['name'] for p in mesh['primitives']} == material_names
    ...
    result = {'draft': draft, 'replay_frames': replay_count,
              ...
              'surface_materials': sorted(material_names),
