...
(async()=>{
 ...
 try{
  ...
  const appleRequests=urls.length;
  await select('apple');await stable();
  assert.equal((await state()).model,'apple');assert.equal((await state()).clips.length,0);
  assert.equal((await page.evaluate(()=>readModelPreview(true))).projectedGroups.length,3);
  assert.equal(await page.locator('[data-preset="红苹果"],[data-preset="青苹果"]').count(),2);
  assert.equal(await page.locator('#play').isDisabled(),true);
  assert.ok((await state()).triangles>400);
  assert.ok((await repairCheck())>0,'apple silhouette and thin stem/leaf use shared coverage');
  assert.ok(urls.slice(appleRequests).every(url=>!url.includes('/assets/models/')),'preview-only apple fetches no game asset');
  const redApple=await png(),redSettings=(await state()).modelSettings;
  await page.locator('[data-preset="青苹果"]').click();await stable();
  const greenSettings=(await state()).modelSettings;
  assert.equal(greenSettings.skinColor,'#9cbb36');assert.equal(await page.locator('#model-skinColor').evaluate(input=>input.value),greenSettings.skinColor);
  assert.notEqual(await png(),redApple);
  const greenPixels=await rgba(),n=(await state()).resolution,center=(Math.floor(n/2)*n+Math.floor(n/2))*4;
  assert.ok(greenPixels[center+1]>greenPixels[center],'green apple center should look green');
  await page.locator('[data-preset="红苹果"]').click();await stable();
  assert.deepEqual((await state()).modelSettings,redSettings);
  const redPixels=await rgba();assert.ok(redPixels[center]>redPixels[center+1],'red apple center should look red');
  await page.screenshot({path:path.join(artifacts,'apple.png'),fullPage:true});
  await page.locator('#model-skinColor').evaluate(picker=>{picker.value='#e09419';picker.dispatchEvent(new Event('input',{bubbles:true}));});await stable();
  assert.equal((await state()).modelSettings.skinColor,'#e09419');assert.notEqual(await png(),redApple);
